" tests/vim_spec/config_spec.vim — Pure VimScript tests for vproj#config#*

let g:CurrentTest = 'config: defaults'
let cfg = vproj#config#Setup({})
...
let cfg = vproj#config#Setup(user)
...
let cfg = vproj#config#Setup({'modes': {'git': {'enabled': v:false}}})
