" tests/vim_spec/events_spec.vim — Pure VimScript tests for vproj#events#*
"
...
let s:ok = v:true
try
  call vproj#events#Emit('nonexistent_event', {})
catch
  let s:ok = v:false
endtry
...
call vproj#events#Clear()
let s:off_id = vproj#events#On('off_test', {data -> 0})
let s:ok = v:true
try
  call vproj#events#Off('off_test', s:off_id)
catch
  let s:ok = v:false
endtry
